...
JNIEXPORT void JNICALL Java_ru_edgar_space_core_ui_hud_Speedometer_sendClick(JNIEnv *env, jobject thiz,
                                                                                    jint buttonid) {
    FLog("pon" + buttonid);
...
int CRadar__SetCoordBlip_hook(int r0, float X, float Y, float Z, int r4, int r5, char* name)
{
	if(pNetGame && !strncmp(name, "CODEWAY", 7))
	{
        ...
        float screenX = X * MULT_X;
        float screenY = Y * MULT_Y;
        //Log("OnClickMap: %f, %f, %f", tmpX, tmpY, tmpZ);

        //Log("OnPlayerClickMap: %f, %f, %f", X, Y, Z);
        //Log("Screen Coordinates: %f, %f", screenX, screenY);
        if(!edgarmap) {
            SetMap(X, Y, Z);
            edgarmap = true;
        } else {
